...
1. **`POST /api/tests/:id/attempts`**
   ...
   - Backend:
     - берёт `user_id` и `username` из `req.user` (payload токена Keycloak);
     - находит тест по `:id` (убедиться, что существует);
     - вычисляет `percent = correctAnswers / questionCount * 100`;
     - создаёт запись в `test_attempts` с текущим временем `created_at`.
